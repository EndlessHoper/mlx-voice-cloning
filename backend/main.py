...
def prepare_recording(raw_path: Path) -> tuple[Path, float, tuple[np.ndarray, int]]:
    """Convert to 24kHz mono WAV via ffmpeg, return (path, duration, (wav_array, sr)) tuple."""
    out_path = REFS_DIR / f"script_ref_{int(time.time())}_{uuid.uuid4().hex[:6]}.wav"
    subprocess.run(
        ["ffmpeg", "-y", "-i", str(raw_path), "-ar", str(TARGET_SR), "-ac", "1",
    ...
    return out_path, duration, (wav, TARGET_SR)
...
def mlx_synthesize_stream(model, text: str, ref_audio_path: str, ref_text: str, **kwargs):
    """Stream speech from MLX backend. Yields np.ndarray chunks (float32, 24kHz mono)."""
    got_any = False
    for result in model.generate(
        ...
        ref_text=ref_text,
        stream=True,
        streaming_interval=0.32,  # ~4 tokens at 12.5Hz = 320ms chunks
        **kwargs,
    ):
        got_any = True
...
def synthesize_from_profile(request: SynthesizeRequest):
    ...
    try:
        ...
        if request.max_new_tokens is not None:
            # mlx-audio v0.4.0 renamed max_new_tokens → max_tokens
            key = "max_tokens" if profile.backend == "mlx" else "max_new_tokens"
            gen_kwargs[key] = request.max_new_tokens
...
def synthesize_stream(request: SynthesizeRequest):
    ...
    gen_kwargs: dict = {}
    if request.max_new_tokens is not None:
        gen_kwargs["max_tokens"] = request.max_new_tokens  # mlx-audio v0.4.0
    if request.repetition_penalty is not None:
        gen_kwargs["repetition_penalty"] = request.repetition_penalty
    if request.temperature is not None:
        gen_kwargs["temperature"] = request.temperature
    ...
    def generate_chunks():
        ...
        model = get_mlx_model(model_id)
        try:
            for chunk in mlx_synthesize_stream(
                model,
                text=text,
                ref_audio_path=profile.ref_audio_path,
                ref_text=profile.ref_text,
