...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function loadConfig() {
      try {
        ...
        // Load demo voices
        try {
          const demoRes = await fetch(apiUrl('/api/demo-voices'))
          if (demoRes.ok) {
            const demoData = await demoRes.json()
            if (Array.isArray(demoData.voices)) setDemoVoices(demoData.voices)
          }
        } catch { /* ignore */ }
  ...
  // ── Use Demo Voice ──
  async function handleDemoVoice(demo: DemoVoice): Promise<void> {
    setError(''); setWarning(''); setSynthStatus('')
    const formData = new FormData()
    formData.append('speaker', demo.id)
    formData.append('device', device)
    formData.append('model_key', modelKey)
    try {
      setTraining(true); setTrainStatus(`Loading ${demo.label}...`)
      appendLog(`Loading demo voice: ${demo.label}`)
      const response = await fetch(apiUrl('/api/train/demo'), { method: 'POST', body: formData })
      const payload = await readResponsePayload(response)
      if (!response.ok) throw new Error(getErrorMessage(payload, 'Demo loading failed.'))
      ...
      const status = getStringField(tp, 'status') || 'Demo voice loaded.'
      setTrainStatus(status)
      appendLog(status, 'success')
      window.setTimeout(() => scrollTo(generateSectionRef), 500)
    } catch (err) {
      const m = err instanceof Error ? err.message : 'Demo loading failed.'
      setError(m); setTrainStatus('Failed.'); appendLog(m, 'error')
  ...
  // ── Spinner (uses memo-stable refs to avoid re-mount flicker on timer ticks) ──
      ...
      <main className="experience">
        ...
        >
          <div className="panel-inner">
            ...
            {demoVoices.length > 0 && (
              <motion.div className="demo-voices" variants={itemReveal}>
                <p className="demo-label">or try a demo voice</p>
                <div className="demo-row">
                  {demoVoices.map((d) => (
                    <button
                      ...
                      disabled={training}
                      onClick={() => void handleDemoVoice(d)}
                    >
                      {d.label}
                    </button>
                  ))}
              ...
              <div className="generation-list">
                {generations.map((gen) => (
                  ...
                  >
                    ...
                    <audio controls src={gen.url} className="audio-el" ref={(el) => { if (el) el.playbackRate = playbackRate }} />
                  </motion.div>
                ))}
              </div>
            ) : (
